% This script generates simulation results depicted in Fig. 6 of the paper
%
...
D2D_delays = [0 5 10 15 20 25 30 35 40];	% Delays in ms for D2D CSI sharing
...
SNRs = [10.^(SNR_dB / 10), ones(1, Nblk + Nbbf + Nbms + Ndly) * 10^(SNR0_dB / 10)];
Blks = [ones(1, Nsnr) * 10^(- Blockage0_dB / 10), ...
        10.^(- Blockage_dB / 10), ...
        ones(1, Nbbf + Nbms + Ndly) * 10^(- Blockage0_dB / 10)];
Bfs = [Bfb0 * ones(1, Nsnr + Nblk), Bfb_vec, Bfb0 * ones(1, Nbms + Ndly)];
Bmss = [Bms0 * ones(1, Nsnr + Nblk + Nbbf + Ndly), Bms_vec];
...
parfor i_case = 1:Ncase
	...
	D2D_delay_ms = D2D_delays(i_case);
...
% Throughput verus delays for D2D CSI sharing
if Ndly > 0
    fig_i = 5;
    figure(fig_i),
    p_handle = plot(D2D_delays, Rsum(schemes_to_show, :), ...
                '-', ...
                'LineWidth', 1.5,...
                'MarkerSize', 9);
    ...
    xlabel('Delays for D2D CSI Exchange (ms)');
